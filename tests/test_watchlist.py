...
def test_get_watchlist_returns_alphabetical(app, sample_user):
    ...
    """
    with app.app_context():
        alpha = Film(title="Alien", year=1979, genre="Horror")
        beta = Film(title="Blade Runner", year=1982, genre="Sci-Fi")
        db.session.add_all([alpha, beta])
        db.session.commit()

        add_to_watchlist(user_id=sample_user, film_id=beta.id)
        add_to_watchlist(user_id=sample_user, film_id=alpha.id)
